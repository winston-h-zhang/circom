...
impl WriteRust for LoadBucket {
    fn produce_rust(&self, producer: &RustProducer, parallel: Option<bool>) -> (Vec<String>, String) {
        ...
        let (mut src_prologue, src_index) = if let LocationRule::Indexed { location, .. } = &self.src {
            ...
            let mut map_access = format!(
                "ctx.circuit.template_to_io_signal[&ctx.components[subcomponents[{}]].template_id][{}].offset",
                cmp_index_ref,
                signal_code,
            );
            if indexes.len() > 0 {
        ...
        let access = match &self.address_type {
            ...
            AddressType::Signal => {
                format!("ctx.signals[signal_start + {src_index}]")
            }
            AddressType::SubcmpSignal { uniform_parallel_value, is_output, .. } => {
                if *is_output {
                    if uniform_parallel_value.is_some() {
			            ...
			            MY_SUBCOMPONENTS, CIRCOM_CALC_WIT, MY_SUBCOMPONENTS)
                            ...
                            prologue.push(format!(
                                "{}->ntcvs.wait(lkt, [{}]() {{return {}->numThread <  {}->maxThread; }});",
                                CIRCOM_CALC_WIT, CIRCOM_CALC_WIT, CIRCOM_CALC_WIT, CIRCOM_CALC_WIT
                            ));
                            prologue.push(format!("ctx->numThread++;"));
                            //prologue.push(format!("printf(\"%i \\n\", ctx->numThread);"));
                            prologue.push(format!("}}"));
                    ...
                    // Case we only know if it is parallel at execution
                        ...
                        prologue.push(format!("if ({}[{}]){{", MY_SUBCOMPONENTS_PARALLEL, cmp_index_ref));
                        ...
                        prologue
                            .push(format!("std::unique_lock<std::mutex> lkt({}->numThreadMutex);", CIRCOM_CALC_WIT));
                        prologue.push(format!(
                            "{}->ntcvs.wait(lkt, [{}]() {{return {}->numThread <  {}->maxThread; }});",
                            CIRCOM_CALC_WIT, CIRCOM_CALC_WIT, CIRCOM_CALC_WIT, CIRCOM_CALC_WIT
                        ));
                ...
                format!("ctx.signals[{} + {}]", sub_cmp_start, src_index)
            }
        };
